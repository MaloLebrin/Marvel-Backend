...
// router.delete('/user/deleteFav', isAuthenticated, async (req, res) => {
//     try {
//         const fav = await Favs.findOneAndDelete({ idMarvel: req.fields.idMarvel })
...
router.get('/user/allFavs', isAuthenticated, async (req, res) => {
    try {
        ...
        if (user) {
            ...
            const count = await Favs.countDocuments(favs); // pour gérer la nombre de doc dans la recherche
